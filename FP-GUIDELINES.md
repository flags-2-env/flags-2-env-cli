...
fully integrated `main`: **196 findings across 35 files and 6,596 lines**. That
baseline includes code recovered from the formerly independent rescue and
generated-contract branches. CI compares against the budget and fails only when
a rule's count *increases*. The inherited backlog blocks nobody; new violations do.
...
|---|---:|---|---|---|
| `RS001` | 77 | warn | immutable values | mutable local binding (`let mut`) |
| `RS003` | 60 | error | typed errors | panic-based control flow (`unwrap`/`expect`/`panic!`) |
| `RS007` | 13 | warn | effects pushed outward | direct stdout/stderr effect in library code |
| `RS004` | 11 | warn | illegal states excluded by types | wildcard match arm defeats exhaustiveness |
| `TS009` | 5 | warn | explicit inputs | ambient impurity (clock / randomness / env) read inside pure code |
| `DA008` | 5 | warn | pure transformations | in-place collection mutation |
| `TS004` | 5 | warn | pure transformations | in-place array mutation |
| `DA005` | 4 | warn | typed errors | `throw` as control flow |
| `TS006` | 4 | warn | typed errors | `throw` as control flow |
| `XX002` | 3 | warn | composition | long function body |
...
*immutable values* · 77 occurrences at baseline
...
*typed errors* · 60 occurrences at baseline
...
*illegal states excluded by types* · 11 occurrences at baseline
...
*effects pushed outward* · 13 occurrences at baseline
